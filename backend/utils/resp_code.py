...
from schemas import SchemasType
from utils import logger


# 因为 data 数据要符合 SchemasType 模型，不符合JSONResponse的序列化
def resp_200(*, data: Union[Type[SchemasType], str, list, dict] = None, msg: str = "Success"):
    logger.info(msg)
    return {'code': 200, 'data': data, 'msg': msg}

# 新的返回数据
def resp_new_200(*, data: Union[str, list, dict] = None, msg: str = "Success"):
    logger.info(msg)
    return {'code': 200, 'result': data, 'message': msg, 'type': 'success'}

# 新的返回数据
def resp_new_str_200(*, data: str = "", msg: str = "Success"):
    logger.info(msg)
    return {'code': 200, 'data': data, 'msg': msg}
