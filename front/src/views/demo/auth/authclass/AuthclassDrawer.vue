...
<script lang="ts">
  ...
  import { authFormSchema } from './authclass.data';
  import { BasicDrawer, useDrawerInner } from '/@/components/Drawer';
  import { updateAuthclass } from '/@/api/demo/authclass';

  //components: BasicTree
  export default defineComponent({
    name: 'AuthclassDrawer',
    ...
    setup(_, { emit }) {
      ...
      const getTitle = computed(() => (!unref(isUpdate) ? '新增授权' : '编辑授权'));
      ...
      async function handleSubmit() {
        try {
          ...
          const dataUpdate = await updateAuthclass(datachange, 'none');
          if (dataUpdate) {
            closeDrawer();
          }
          emit('success');
