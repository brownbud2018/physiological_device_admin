...
<script lang="ts">
  ...
  import { getUserBloodoxygenList2 } from '/@/api/demo/user';
    ...
    setup(props: any, context: any) {
      ...
      watch(
        ...
        (newValue, oldValue) => {
          ...
          let this_data_id = props.data.oxy_data_id;
          if (this_data_id != undefined) {
            const promisedata = new Promise((resolve) => {
              const data1 = getUserBloodoxygenList2(user_id)
                .then((res) => {
                  let j = 0;
                  for (let i = 0; i < res.data.length; i++) {
                    if (
                      res.data[i].spo != undefined &&
                      res.data[i].spo != '' &&
                      res.data[i].spo != null
                    ) {
                      dateData[j] = res.data[i].createtime;
                      spoData[j] = res.data[i].spo;
                      pulseData[i] = res.data[i].pulse;
                      j = j + 1;
                    }
                  }
                  if (spoData.length > 0) {
                    titleData = '血氧';
                    subData = '血氧饱和度正常范围：大于94，脉率正常范围：50~120bpm';
                    leftData = 'left';
                    topData = 'top';
                    title = {
                      text: titleData,
