...
<script lang="ts">
  ...
  import { getUserBloodoxygenList3 } from '/@/api/demo/user'; //
  //参数 数据类型配置
  interface DataProps {
    son: any;
    key: number;
  ...
  export default defineComponent({
    ...
    setup(props: any, context: any) {
      ...
      watch(
        () => props.data,
        (newValue, oldValue) => {
          //console.log('echartsBloodoxygen3', props.data);
          let this_data_id = props.data.this_data_id;
          //console.log('this_data_id', this_data_id);
          const promisedata = new Promise((resolve) => {
            const data1 = getUserBloodoxygenList3(this_data_id)
              .then((res) => {
                if (res.data.id > 0) {
                  endData = res.data.bloodoxygenurl;
                ...
                setHData(title, endData);
              })
              .catch(() => {
                createMessage.error('查询用户血氧失败');
                title = {
                  text: titleData,
                  left: leftData,
                  top: topData,
